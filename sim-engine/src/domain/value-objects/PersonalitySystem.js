...
class PersonalitySystem {
    ...
    saveToLocalStorage(key = 'personalitySystem') {
        try {
            const data = this.toJSON();
            localStorage.setItem(key, JSON.stringify(data));
            return true;
        } catch (error) {
            console.error('Failed to save PersonalitySystem to localStorage:', error);
            return false;
        }
    }

    static loadFromLocalStorage(key = 'personalitySystem') {
        try {
            const data = localStorage.getItem(key);
            if (!data) {
                return null;
            }
            return PersonalitySystem.fromJSON(JSON.parse(data));
        } catch (error) {
            console.error('Failed to load PersonalitySystem from localStorage:', error);
            return null;
        }
    }

    clearLocalStorage(key = 'personalitySystem') {
        try {
            localStorage.removeItem(key);
            return true;
        } catch (error) {
            console.error('Failed to clear PersonalitySystem from localStorage:', error);
            return false;
        }
    }
}

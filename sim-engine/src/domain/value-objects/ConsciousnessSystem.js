...
class ConsciousnessSystem {
    ...
    // Emotional System Integration Methods
    getCurrentEmotionalState(character) {
        ...
        const freq = character.consciousness.frequency || 40;
        const coherence = character.consciousness.coherence || 0.7;
        
        // Map frequency to practical emotions
        let primaryEmotion;
        if (freq < 30) primaryEmotion = 'tired';
        else if (freq < 50) primaryEmotion = 'content';
        else if (freq < 70) primaryEmotion = 'alert';
        else primaryEmotion = 'energized';

        // Calculate intensity based on coherence
        const intensity = Math.min(1.0, Math.max(0.1, coherence));

        // Extract active emotional modifiers
        const modifiers = [];
        if (character.consciousness.emotionalModifiers) {
            character.consciousness.emotionalModifiers.forEach((modifier, type) => {
                modifiers.push({
                    type: type,
    ...
    applyEmotionalEvent(character, eventType, intensity = 1.0, duration = 5) {
        if (!character) {
            return character;
        }
        ...
        // Create new character instance to avoid mutation
        const updatedCharacter = {
            ...character,
            consciousness: {
                ...character.consciousness,
                emotionalModifiers: new Map(character.consciousness.emotionalModifiers || new Map())
            }
        };

        // Add or update emotional modifier
        updatedCharacter.consciousness.emotionalModifiers.set(eventType, {
            intensity,
            duration,
            type: eventType,
            timestamp: Date.now()

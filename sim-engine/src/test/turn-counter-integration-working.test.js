...
// These tests focus on what actually works in the current implementation
...
// Mock the RunTick function to provide predictable behavior
...
describe('Turn Counter Integration Tests - Working Implementation', () => {
  ...
  describe('Basic Turn Counter Functionality', () => {
    ...
    test('should display turn counter with proper formatting', async () => {
      ...
      await waitFor(() => {
        ...
        expect(turnCounter.querySelector('span')).toHaveClass('text-sm', 'text-gray-500', 'dark:text-gray-400');
      });
    });
  ...
  describe('Service Integration', () => {
    ...
    test('should handle service initialization', async () => {
      const initializeSpy = jest.spyOn(SimulationService, 'initialize');
      ...
      // Service should be initialized when component mounts
      await waitFor(() => {
        expect(screen.getByTestId('turn-counter')).toHaveTextContent('Turn: 0');
      });
  ...
  describe('Persistence Integration', () => {
    test('should attempt to save state to localStorage', async () => {
      const TestComponent = () => {
  ...
  describe('Real-time Simulation Behavior', () => {
